...
func (o *order) generateFollowup(ctx context.Context, questions, delegation, dns string, countQ int) {
	...
	if o.hosting == "" {
		...
		txt.WriteString("```bash\n")
		txt.WriteString("# ensure the .ssh directory exists\n")
		txt.WriteString("mkdir -p " + strings.TrimSuffix(authorizedkeys, "/authorized_keys") + "\n")
		txt.WriteString("# ensure the authorized_keys file exists\n")
		txt.WriteString("touch " + authorizedkeys + "\n")
		txt.WriteString("# add the key to the authorized_keys file\n")
		txt.WriteString("echo '" + pubkey + "' >> " + authorizedkeys + "\n")
		txt.WriteString("# ensure the correct permissions are set on the authorized_keys file\n")
		txt.WriteString("chmod 600 " + authorizedkeys + "\n")
		txt.WriteString("# ensure the correct permissions are set on the .ssh directory\n")
		txt.WriteString("chmod 700 " + strings.TrimSuffix(authorizedkeys, "/authorized_keys") + "\n")
		txt.WriteString("# ensure the correct ownership is set on the .ssh directory\n")
		txt.WriteString("chown -hR " + o.get("ssh-user") + ":" + o.get("ssh-user") + " " + strings.TrimSuffix(authorizedkeys, "/authorized_keys") + "\n")
		txt.WriteString("```\n\n")
