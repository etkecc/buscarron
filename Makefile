...
CI_REGISTRY_IMAGE ?= registry.gitlab.com/etke.cc/buscarron
REGISTRY_IMAGE ?= registry.etke.cc/etke.cc/buscarron
...
# run buscarron, note: make doesn't understand exit code 130 and sets it == 1
run:
	@go run ${BUILDFLAGS} ./cmd || exit 0

# build buscarron
build:
	go build ${BUILDFLAGS} -v -o buscarron -ldflags "-X main.version=${CI_COMMIT_TAG}" ./cmd
